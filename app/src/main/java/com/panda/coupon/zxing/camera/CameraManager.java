...
public final class CameraManager {
    ...
    static final int MIN_FRAME_WIDTH = 480;
    static final int MIN_FRAME_HEIGHT = 480;
    static final int MAX_FRAME_WIDTH = 1440;
    static final int MAX_FRAME_HEIGHT = 1440;
...
//            FlashlightManager.enableFlashlight();
    ...
    public void startPreview() {
        if (camera != null && !previewing) {
            try {
                camera.startPreview();
            } catch (Exception e) { // 如果摄像头未授予权限、被占用或者不存在，未检查异常将会导致应用程序被系统强行关闭。
                //               ToastTools.makeText(context, "摄像头未授予权限、被占用或者不存在", Toast.LENGTH_LONG).show();
                //              JuMeiLogMng.getInstance().e(TAG, "打开摄像头异常 ： " + e.getMessage());
            }
            previewing = true;
        }
    }
    ...
    public void stopPreview() {
        if (camera != null && previewing) {
            ...
            try {
                ...
                //              ToastTools.makeText(context, "摄像头未授予权限、被占用或者不存在", Toast.LENGTH_LONG).show();
                //          JuMeiLogMng.getInstance().e(TAG, "关闭摄像头异常 ： " + e.getMessage());
            }
...
//      rect.bottom = rect.bottom * cameraResolution.y / screenResolution.y;
            ...
            if(DensityUtils.dip2px(60)<rect.top) {
                rect.top = rect.top - DensityUtils.dip2px(60);
            }
    ...
    public void setZoomLarge(){
        ...
        try {
            ...
            if (parameters.isZoomSupported()) {
                if (zoom == 0) {
                    zoom = maxZoom / 3;
                } else if (zoom <= maxZoom - maxZoom/6) {
                    zoom = zoom + maxZoom/6;
                } else {
                    zoom = maxZoom;
                }
