...
import com.panda.coupon.zxing.utils.QrUtils;

/**
 * 解析图像二维码线程
 */
public class DecodeImageThread implements Runnable {
    static final int MAX_PICTURE_PIXEL = 512;
    byte[] mData;
    int mWidth;
    int mHeight;
    String mImgPath;
    DecodeImageCallback mCallback;
    ...
    @Override
    public void run() {
        Result result = QrUtils.syncDecodeQRCode(QrUtils.getDecodeAbleBitmap(mImgPath));
        if(result == null) {
            if (null == mData) {
            ...
            result = QrUtils.decodeImage(mData, mWidth, mHeight);
        }
        if (null != mCallback) {
            if (null != result) {
